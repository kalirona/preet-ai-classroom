...
export default function SettingsView({ currentUser, activeCommunity, onUpdateCommunity }: SettingsViewProps) {
  ...
  const tabs = [
    { id: "general", name: "A. General Branding", icon: Palette },
    { id: "membership", name: "B. Membership & Plans", icon: CreditCard },
    { id: "courses", name: "C. Course Drips & Certificates", icon: PlayCircle },
    { id: "community", name: "D. Community Channels", icon: Sliders },
    { id: "gamification", name: "E. Gamification (XP & Badges)", icon: Award },
    { id: "integrations", name: "F. Integrations (Stripe & Webhooks)", icon: Link2 },
    { id: "email", name: "G. SMTP & Resend Templates", icon: Mail },
    { id: "security", name: "H. Security & 2FA Logs", icon: ShieldCheck },
    { id: "team", name: "I. Team Roles Editor", icon: Users },
  ];
  ...
  return (
    <div className="h-full flex flex-col lg:flex-row bg-[#F8F9FB] overflow-hidden" id="deep-settings-dashboard">
      ...
      <div className="w-full lg:w-72 border-r border-[#E5E7EB] bg-white flex flex-col shrink-0">
        <div className="p-5 border-b border-[#E5E7EB]">
          <h3 className="text-xs font-bold uppercase tracking-widest text-indigo-600 font-mono">Platform Admin</h3>
          <h2 className="text-lg font-bold text-gray-900 font-display mt-0.5">SaaS System parameters</h2>
          ...
          {tabs.map((t) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center justify-between px-3.5 py-3 rounded-xl text-xs font-semibold text-left transition cursor-pointer ${
                  isSel 
              ...
              >
                <div className="flex items-center gap-3">
                  <Icon className={`w-4 h-4 ${isSel ? "text-indigo-650" : "text-gray-400"}`} />
                  <span>{t.name}</span>
                </div>
      ...
      <div className="flex-1 p-6 overflow-y-auto space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-[#E5E7EB] p-5 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <span className="text-[9px] uppercase font-mono tracking-widest bg-indigo-50 px-2 py-0.5 rounded text-indigo-700 font-bold border border-indigo-100">
          ...
          <button
            ...
            disabled={isSaving}
            className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white text-xs font-bold rounded-xl transition shadow-sm flex items-center gap-1.5 cursor-pointer"
          >
        ...
        {activeSubTab === "general" && (
          <div className="bg-white rounded-3xl border border-[#E5E7EB] p-6 space-y-6 shadow-sm">
            <div>
              <h4 className="text-xs font-bold text-gray-800 uppercase tracking-widest font-mono mb-1">A. Spaces branding parameters</h4>
              <p className="text-[11px] text-gray-450">Change your community name, URL handles, theme colors, and custom domains.</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div>
                <label className="block text-xs font-bold text-gray-750 mb-1">Space App Name</label>
                <input
                  type="text"
                  value={commName}
                  onChange={(e) => setCommName(e.target.value)}
                  className="w-full border border-gray-200 rounded-xl px-3.5 py-2.5 text-xs text-gray-950 focus:outline-none focus:ring-1 focus:ring-indigo-500"
                />
              </div>

              <div>
                <label className="block text-xs font-bold text-gray-750 mb-1">Subdomain routing handle</label>
                <div className="flex items-center border border-gray-200 rounded-xl bg-gray-50 px-3 py-2">
                  ...
                  />
                  <span className="text-[11px] text-gray-400 font-mono">.skool.sh</span>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              <div>
                <label className="block text-xs font-bold text-gray-750 mb-1">Branding Icon Symbol</label>
                <input
                  type="text"
                  value={commLogo}
              ...
              <div>
                <label className="block text-xs font-bold text-gray-750 mb-1">Branding Theme Accent</label>
                <select
                  value={commPrimary}
              ...
              <div>
                <label className="block text-xs font-bold text-gray-750 mb-1">SEO Custom white-label domain name</label>
        ...
        {activeSubTab === "membership" && (
          <div className="bg-white rounded-3xl border border-[#E5E7EB] p-6 space-y-6 shadow-sm">
            <div>
              ...
              <p className="text-[11px] text-gray-450">Set up price tiering structures, subscription logic, and trial period variables.</p>
            </div>
            ...
            {isPaidJoint && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5 animate-in fade-in duration-150">
                <div>
                  <label className="block text-xs font-bold text-gray-750 mb-1">SaaS Monthly Subscription price (USD)</label>
                  <div className="flex items-center border border-gray-200 rounded-xl bg-gray-50 px-3 py-2">
                    ...
                    <input
                      ...
                      className="w-full bg-transparent border-none text-xs text-gray-950 focus:outline-none font-mono"
                    />
                    <span className="text-gray-450 text-[10px] uppercase font-mono">/ Month</span>
                  </div>
                </div>

                <div>
                  <label className="block text-xs font-bold text-gray-750 mb-1">simulated checkout Trial Limits</label>
                  <select
                    value={trialDays}
        ...
        {activeSubTab === "courses" && (
          <div className="bg-white rounded-3xl border border-[#E5E7EB] p-6 space-y-6 shadow-sm">
            <div>
              ...
              <p className="text-[11px] text-gray-450">Set rules for drip lesson distributions, final course completion conditions, and certificate generations.</p>
            </div>
        ...
        {activeSubTab === "community" && (
          <div className="bg-white rounded-3xl border border-[#E5E7EB] p-6 space-y-6 shadow-sm">
            <div>
              ...
              <p className="text-[11px] text-gray-450">Add new sidebar discussion tags and restrict posting roles.</p>
            </div>

            <div className="space-y-4">
              <label className="block text-xs font-bold text-gray-750">Active tags/channels registry</label>
              <div className="space-y-2.5 max-h-48 overflow-y-auto border border-gray-100 p-3 rounded-2xl bg-gray-55/40">
                {communityChannels.map((c) => (
                  <div key={c.id} className="flex justify-between items-center bg-white border border-[#E5E7EB] px-3.5 py-2 rounded-xl text-xs">
        ...
        {activeSubTab === "gamification" && (
          <div className="bg-white rounded-3xl border border-[#E5E7EB] p-6 space-y-6 shadow-sm">
            <div>
              ...
              <p className="text-[11px] text-gray-450">Set up community participation metrics to accelerate engagement pacing automatically.</p>
            </div>
        ...
        {activeSubTab === "integrations" && (
          <div className="bg-white rounded-3xl border border-[#E5E7EB] p-6 space-y-6 shadow-sm">
            <div>
              ...
              <p className="text-[11px] text-gray-450">Stripe configurations for White-label domains and webhook dispatch hooks.</p>
            </div>
        ...
        {activeSubTab === "email" && (
          <div className="bg-white rounded-3xl border border-[#E5E7EB] p-6 space-y-6 shadow-sm">
            <div>
              ...
              <p className="text-[11px] text-gray-450">Configure email integrations so notifications arrive in inbox folders instantly.</p>
            </div>
        ...
        {activeSubTab === "security" && (
          <div className="bg-white rounded-3xl border border-[#E5E7EB] p-6 space-y-6 shadow-sm">
            <div>
              ...
              <p className="text-[11px] text-gray-450">Audits current security vectors, session telemetry logs, and developer credentials.</p>
            </div>
        ...
        {activeSubTab === "team" && (
          <div className="bg-white rounded-3xl border border-[#E5E7EB] p-6 space-y-6 shadow-sm">
            <div>
              ...
              <p className="text-[11px] text-gray-450">Invite colleagues to take over instructor or moderator privileges in your workspace.</p>
            </div>

            <div className="space-y-4">
              <div className="space-y-2 max-h-40 overflow-y-auto">
                {teamInvites.map((inv, idx) => (
                  <div key={idx} className="flex justify-between items-center px-4 py-2 bg-gray-50/60 rounded-xl border border-gray-100 text-xs">
                    <div>
              ...
              <div className="flex gap-2 items-end">
                <div className="flex-1">
                  <label className="block text-xs font-bold text-gray-750 mb-1">Invite team email</label>
                  <input
                    type="email"
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-750 mb-1">Select Role</label>
                  <select
                    value={newTeamRole}
